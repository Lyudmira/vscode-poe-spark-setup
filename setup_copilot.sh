...
if LEGACY_OLD in content:
    content = content.replace(LEGACY_OLD, LEGACY_NEW, 1)
elif CURRENT_OLD in content:
    content = content.replace(CURRENT_OLD, CURRENT_NEW, 1)
else:
...
with open(ext_js) as f:
    verify = f.read()
assert PATCH_MARKER in verify, '验证失败：Patch 2 标记未找到'
print('✓ 验证通过')
PYEOF
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "Copilot Chat 扩展升级后，top_p 补丁会丢失，需要重新运行：bash setup_copilot.sh"
echo ""
echo "检查方式："
echo ""
...
echo "  grep -c '__poeToolIds' \"\$EXT_JS\" || echo 'Patch 2 MISSING'  # 输出0表示未找到即丢失"
