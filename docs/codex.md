...
如果降低到 `medium` 后仍然出现模型反复执行无意义命令、坚持将简单任务复杂化、进行大量无谓的文件浏览和指令运行的情况，切换到 **Plan mode** 可以有效解决。Plan mode 会先让模型输出完整计划等待确认，再逐步执行，避免模型"失控自走"。

### 提交消息后完全没有反应（动态 limit）

如果输入指令提交后模型完全没有任何反应，可能是触发了该模型的动态请求限制（非固定频率，随负载变化）。**解决方法**：直接 Edit 该条消息，然后重新 Submit，不需要修改内容。平均三到七次尝试即可成功发出。
